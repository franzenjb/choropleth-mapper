class ChoroplethMapper {
    ...
    async fetchGeographicData(geoLevel, stateFilter) {
        ...
        // For ZIP codes without state filter, use TIGER and filter for detected states
        if (!stateFilter && geoLevel === 'zip') {
            ...
            if (detectedStates.size > 0) {
                ...
                // Fetch from TIGER without state filter (gets all)
                await this.fetchSingleStateData(geoLevel, null);
                
                // Now filter to only the ZIP codes we need based on detected states
                if (this.geoData && this.geoData.features) {
                    const originalCount = this.geoData.features.length;
                    
                    // Build all valid ZIP ranges for detected states
                    const allRanges = [];
                    ...
                    // Filter features to only those matching our states
                    this.geoData.features = this.geoData.features.filter(f => {
                        // TIGER uses different field names
    ...
    async fetchSingleStateData(geoLevel, stateFilter) {
        ...
        // For ZIP codes, use TIGER directly as it's more reliable
        if (geoLevel === 'zip') {
            console.log(`Using TIGER Census API for ZIP codes`);
            const tigerUrl = this.getTigerUrl(geoLevel, year, stateFilter);
            console.log(`TIGER URL: ${tigerUrl}`);
            
            try {
                const response = await fetch(tigerUrl);
                if (!response.ok) {
                    throw new Error(`TIGER API returned ${response.status}`);
                }
                ...
                if (this.geoData && this.geoData.features) {
                    console.log(`Loaded ${this.geoData.features.length} ZIP codes from TIGER`);
                    
                    // If we have a state filter, filter the results
                    if (stateFilter) {
                ...
                // Continue to try ArcGIS as fallback
            }
        }
